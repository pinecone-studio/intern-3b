...
export const mockExamInfos: ExamInfo[] = [
  {
    ...
    strategy: "Лекцийн материалыг сайн давтаж, лабораторийн ажлуудыг нягт хараарай.",
    difficulty: "Дунд",
    questionExamples: ["ООП-ийн үндсэн ойлголтууд", "Өгөгдлийн бүтцийн тодорхойлолт"],
...
export default function Exam() {
  ...
  const handleRevealExam = (examId: number) => {
    setExamInfos((prev) => prev.map((exam) => (exam.id === examId ? { ...exam, revealed: true } : exam)))
    toast({
        ...
        prev.map((exam) =>
          exam.id === examId
            ? {
                ...
                dislikes: voteType === "dislike" ? exam.dislikes - 1 : exam.dislikes,
              }
            : exam,
        ),
      )
      ...
      setExamInfos((prev) =>
        prev.map((exam) =>
          exam.id === examId
            ? {
                ...exam,
                likes: voteType === "like" ? exam.likes + 1 : currentVote === "like" ? exam.likes - 1 : exam.likes,
                dislikes:
                  voteType === "dislike"
                    ? exam.dislikes + 1
                    : currentVote === "dislike"
  ...
  if (showAddExam) {
    return <AddExamForm onBack={() => setShowAddExam(false)} onSubmit={() => setShowAddExam(false)} />
  }

  return (
    <div className="flex flex-col min-h-screen bg-background max-w-md mx-auto">
      <AppHeader title="Шалгалтын мэдээлэл" onBack={() => router.push(`/courses/${courseId}`)} />

      <main className="flex-1 overflow-y-auto pb-20">
        {examInfos.length === 0 ? (
          <div className="flex flex-col items-center justify-center h-64 px-4 text-center">
            <FileText className="h-12 w-12 text-muted-foreground/40 mb-3" />
            <h3 className="text-sm font-semibold text-foreground mb-1">Шалгалтын мэдээлэл байхгүй</h3>
            <p className="text-xs text-muted-foreground">Анхны мэдээллийг нэмж бусдад туслаарай</p>
          </div>
        ) : (
          <ExamInfoList
            examInfos={examInfos}
      ...
      <div className="fixed bottom-0 left-0 right-0 p-4 bg-card border-t border-border max-w-md mx-auto">
        <AppButton variant="primary" className="w-full" onClick={() => setShowAddExam(true)}>
