  ...
  return (
    <main className="max-w-6xl mx-auto px-6 py-10 ">
      ...
      <div className="bg-white border rounded p-6 shadow-sm">
        ...
        {/* Clear button */}
        <div className="mt-4">
          <button onClick={handleClear} className="text-emerald-600 font-bold text-sm hover:text-emerald-700 transition-colors flex items-center gap-1 group">
      ...
      {isProgressComplete && !selectedTopicId && (
        <div className="animate-in mt-20 fade-in slide-in-from-bottom-2 duration-500">
               <div className="flex items-center justify-between mb-8">
                  <h2 className="text-2xl font-black text-slate-800">Ерөнхий сэдвүүд</h2>
      ...
      {selectedTopicId && selectedTopic && (
        ...
        <div>
          <h2 className="text-3xl font-black text-slate-900">{selectedTopic.name}</h2>
          <p className="text-slate-500 font-medium">Нийт {selectedTopic.lessons.length} дэд сэдэв байна</p>
        </div>
      </div>
           <div className="lg:col-span-2 space-y-4">
          {selectedTopic.lessons.map((lesson) => (
             <div key={lesson.id} className="p-6 rounded-2xl border border-slate-100 bg-white hover:border-[#B0DAC8] hover:shadow-lg hover:shadow-[#B0DAC8]/10 transition-all group flex flex-col sm:flex-row sm:items-center justify-between gap-6">
                    <div className="flex items-center gap-6">
                      ...
                      <div>
                        <h4 className="text-lg font-bold text-slate-800 mb-1 group-hover:text-[#5E8C78] transition-colors">{lesson.name}</h4>
                        <div className="flex items-center gap-4 mt-1">
                          <span className="flex items-center gap-1.5 text-sm font-semibold text-slate-400">{lesson.teacherCount} багш</span>
                          <span className="flex items-center gap-1.5 text-xs font-black text-emerald-600 uppercase tracking-wider bg-emerald-50 px-2 py-0.5 rounded-md"> Батлагдсан</span>
                        </div>
                      </div>
                    </div>
                    <div className="flex items-center gap-3">
                      <Button variant="outline" className="h-11 px-6 font-bold" >Материал нэмэх</Button>
                      <Button variant="primary" className="h-11 px-8 font-bold" >Үзэх </Button>
                    </div>
                  </div>
        ...
        <div className="flex flex-col items-center justify-center py-32 text-center space-y-8">
          ...
          <div className="max-w-sm space-y-3">
            ...
            <p className="text-gray-400">
              Хайлтаа эхлүүлэхийн тулд хичээл, анги болон улирлын мэдээллээ сонгоно уу.
            </p>
          </div>
        </div>
      )}
