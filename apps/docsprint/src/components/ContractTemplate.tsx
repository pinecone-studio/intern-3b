  ...
  return (
    <div className="flex flex-col gap-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4">
        ...
        <div className="flex items-center gap-3">
          <button className="flex items-center gap-2 px-4 py-2.5 text-slate-600 hover:bg-white border border-transparent hover:border-slate-200 rounded-xl transition-all text-sm font-bold">
            <RotateCcw className="w-4 h-4" /> Сэргээх
          </button>
          <button className="bg-[#005bb7] hover:bg-[#004a96] text-white px-6 py-2.5 rounded-xl text-sm font-bold transition-all shadow-lg shadow-blue-100 flex items-center gap-2 active:scale-95">
            <Save className="w-4 h-4" /> Хадгалах
          </button>
        </div>
      </div>
          ...
          <nav className="flex-1 overflow-y-auto p-2 space-y-1 custom-scrollbar">
            {templateOptions.map((item) => (
              ...
              >
                <div className="flex items-center gap-3">
                  <item.icon
                    className={`w-4 h-4 ${selectedId === item.id ? 'text-[#005bb7]' : 'text-slate-400 group-hover:text-slate-600'}`}
                  />
                  {item.name}
                </div>
                {selectedId === item.id && (
        ...
        <div className="flex-1 flex flex-col bg-slate-100 rounded-3xl border border-slate-200 overflow-hidden relative shadow-inner">
          ...
          <div className="flex-1 overflow-y-auto p-10 flex justify-center bg-[url('https://www.toptal.com/designers/subtlepatterns/patterns/grid.png')] bg-fixed custom-scrollbar">
            <div className="relative group">
              ...
              {/* Actual Paper */}
              <div className="relative w-[750px] min-h-[960px] bg-white shadow-[0_20px_50px_rgba(0,0,0,0.05)] border border-slate-200 p-20 flex flex-col transition-all">
                {/* Document Header */}
