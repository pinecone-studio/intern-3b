...
export function CourseLessonTree({ course, completedLessons, onLessonClick, onBack }: CourseLessonTreeProps) {
  const getNextLesson = () => {
    const firstIncomplete = course.lessons.find((lesson) => !completedLessons.includes(lesson.id))
    ...
    <div className="max-w-3xl mx-auto">
      ...
      <motion.div initial={{ y: -20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} className="mb-8">
        ...
        <div className="flex items-center gap-4 text-sm">
          ...
          <div className="text-primary font-semibold">+{completedLessons.length * 50} XP олсон</div>
        </div>
      </motion.div>
            ...
            return (
              ...
              >
                ...
                >
                  ...
                  {isCurrent && <Play className="w-10 h-10 text-white fill-white" />}
                ...
                >
                  ...
                  <p className="text-sm text-muted-foreground mt-1">{lesson.descriptionMn}</p>
                  {!isLocked && <div className="mt-2 text-xs font-semibold text-primary">+50 XP</div>}
                </motion.div>
              </motion.div>
            )
          })}
